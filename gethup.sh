...
#Create Directory
if [ ! -d "${HOME}/iodt-node" ]; then
  mkdir -p ${HOME}/iodt-node
  echo "Created Directory"
fi

root= ${HOME}/iodt-node  # base directory to use for datadir and logs
#shift
dd=01  # double digit instance id like 00 01 02
#shift
...
GETH='geth --nodiscover --networkid 1999 --genesis ./genesis.json'
echo $GETH
...
echo "$GETH --datadir=$datadir \
  ...
  --unlock=$BZZKEY \
  --password=<(echo -n $dd) \
  --rpc --rpcapi 'db,admin,eth,net,web3' --rpcport=$rpcport --rpccorsdomain='*' $* \
